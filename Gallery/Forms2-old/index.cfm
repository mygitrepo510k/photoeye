...
<cfif xPan IS 1>
	var ContainerHeight = (<cfoutput>#xCounter#</cfoutput>*125)+'px';
<cfelse>
	var ContainerHeight = (Math.ceil(<cfoutput>#xCounter#</cfoutput>/7)*90)+'px';
</cfif>
$('Img_Container').setStyle({height:ContainerHeight});
window.name = 'PEMain';
</script>
<div id="TogHowTo" onclick="$('HowTo').toggle()" style="text-align:center; cursor:pointer;color:#A8905C;margin-top:50px;text-transform:uppercase;margin-bottom:5px;font-size:10px">How to use our image viewer</div>
