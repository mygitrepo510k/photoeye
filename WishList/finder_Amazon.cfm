...
<CFIF IsDefined("form.CustNum")>
	<CFIF form.CustNum is "myWishList">
		<CFLOCATION URL="index_Amazon.cfm?my=1">
	</CFIF>
</CFIF>
...
<BR>
<BR>
<FORM METHOD="POST" ACTION="#xURLHOMe#/wishlist/invite_Amazon.cfm?get=1"><input TYPE="SUBMIT" CLASS="Button" value="Invite #trim(form.email)# to make a wish list!" name="Invite" style="font-size: 10px">
	...
	<CFELSE>
		...
		<CFIF xWishListID gt 0>
		   ...
		   WHERE WishListID = #qWishList.WIshListID#
			...
			<CFLOCATION url="../templates/Wishlist_Amazon.cfm">
		<CFELSE>
			<CFSET xErrorMessage = '<DIV ALIGN="CENTER">#qFindUser.FirstName# #qFindUser.LastName# (#trim(form.email)#) has no wish list.
<BR>
<BR>
<FORM METHOD="POST" ACTION="#xURLHOMe#/wishlist/invite_Amazon.cfm?get=1"><input TYPE="SUBMIT" CLASS="Button" value="Invite #trim(qFindUser.FirstName)# to make one!" name="Invite" style="font-size: 10px">
<INPUT TYPE="HIDDEN" NAME="toEMAIL" VALUE="#trim(form.email)#">
</FORM></DIV>'>
		</CFIF>
...
<CFINCLUDE template="index_Amazon.cfm">